//! Self-contained reverse-geocoding engine over a pre-built `geo.bin`.
//!
//! Single-file rewrite of the genom lookup pipeline, optimized for
//! in-memory size and load/lookup speed:
//!
//! - Owns the file as `Box<[u8]>` (no leak, no mmap, one syscall).
//! - Sorted `Box<[(u32, u32)]>` grid indexes + binary search (no hashmaps).
//! - Varint/zigzag decode on demand; string + ps-offsets read unaligned on demand.
//! - Enrichment tables stored as sorted `&[(u16, ...)]` slices with binary search.
...
/// Enriched reverse-geocoding result. Field-compatible with `genom::Place`.
#[derive(Debug, Clone)]
pub struct Place {
...
/// Reverse-geocoder backed by a `geo.bin` loaded from disk.
pub struct Geo {
    bytes: Box<[u8]>,
...
impl Geo {
    /// Load and parse a `geo.bin` file. One read + one parse pass.
    pub fn open(path: impl AsRef<Path>) -> io::Result<Self> {
        let bytes = fs::read(path)?.into_boxed_slice();
        Self::parse(bytes).ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "bad geo.bin"))
    }

    fn parse(bytes: Box<[u8]>) -> Option<Self> {
        if bytes.len() < 64 || &bytes[..4] != b"GEO1" {
            return None;
        }
        let d = &bytes[..];
        let off_str = u32le(d, 8) as usize;
        let off_cc = u32le(d, 16) as usize;
        let off_grid = u32le(d, 24) as usize;
        let off_cities = u32le(d, 32) as usize;
        let len_cities = u32le(d, 36) as usize;
        let off_pdir = u32le(d, 40) as usize;
        let off_postal = u32le(d, 48) as usize;

        let str_count = u32le(d, off_str) as usize;
        let str_offsets_off = off_str + 4;
        let str_body_off = str_offsets_off + 4 * (str_count + 1);
        let cc_off = off_cc + 4;

        let grid = decode_grid(d, off_grid);

        let pdir_count = u32le(d, off_pdir) as usize;
        let mut postal: Vec<PCountry> = Vec::with_capacity(pdir_count);
        let mut p = off_pdir + 4;
        for _ in 0..pdir_count {
            let cc = u16::from_le_bytes([d[p], d[p + 1]]);
            let start = u32le(d, p + 4) as usize;
            let end = u32le(d, p + 8) as usize;
            p += 12;
            postal.push(parse_pcountry(d, cc, off_postal + start, off_postal + end));
        }
    ...
    /// Reverse-geocode `(latitude, longitude)`. Identical output to `genom::lookup`.
...
fn parse_pcountry(d: &[u8], cc: u16, start: usize, end: usize) -> PCountry {
    let sec = &d[start..end];
    let mut p = 0;
...
fn scan_city_cell(buf: &[u8], off: usize, lat_q: i64, lon_q: i64, best: &mut Option<(i64, CityHit)>) {
    let mut i = off;
